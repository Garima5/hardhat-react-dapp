...
function App() {
  ...
  async function requestAccount(){
    ...
    // Ask user to connect toi Metamask if not already connected
  ...
  // functions for token's front end
  ...
  // ffunction for sending the coins
  async function sendCoins(){
    if (typeof window.ethereum !== 'undefined'){
      await requestAccount() // make sure they have given us access to their wallet
      const provider = new ethers.providers.Web3Provider(window.ethereum)
  ...
  return (
    <div className="App">
      ...
      <button onClick = {getBalance}>Get Ballance</button>
